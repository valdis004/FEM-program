...
void Mesh::createDefaultMesh() {
  ...
  const ElementType type = ElementType::MITC4MY;
  AreaLoadPlate load = AreaLoadPlate(-100);
  ElementData data = ElementProvider::props[type];
  ...
  for (int l = 1; l <= steps; l++) {
    for (int k = 1; k <= steps; k++) {
      ...
      Node node;
      const int count = data.nodesCount;
      ...
      for (int j = 0; j < count; j++) {
        ...
        Point3 pointForNode =
            data.getPointFromIndex(j, point0, step, cosA, sinA);
        ...
        if (flag) {
          node =
              Node(possibleNode.point, possibleNode.dofCount, possibleNode.id);
        } else {
          int index = 0;
          if (j != 0)
            index = maxNodeIndexInList(this->nodes) + 1;

          node = Node(pointForNode, data.dofCount, index);
      ...
      this->elements.push_back(
          AbstractElement::create(elementsCount, type, nodesToElem, ptr));
      elementsCount++;
    }
  }
}